...
class TankEnv:
    ...
    def reset(self, log_data):
        return {"status": "success", "control": "reset"}
    
    def update_state(self, log_data):
        # 탱크 위치
        self.tank_x = float(log_data.get("playerPos", {}).get("x", 0.0))
        self.tank_y = float(log_data.get("playerPos", {}).get("z", 0.0)) # y축은 높이이므로 z축을 사용용
        # 포탑 각도
        self.turret_x = float(log_data.get("playerTurretX", 0.0))
        self.turret_y = float(log_data.get("playerTurretY", 0.0))
        # 적 위치
        self.enemy_x = float(log_data.get("enemyPos", {}).get("x", 0.0))
        self.enemy_y = float(log_data.get("enemyPos", {}).get("z", 0.0))
        # 적중 여부
        self.hit = 0.0
        # 발사 쿨타임
        ...
        self.time_since_last_fire = self.current_time - self.last_fire_time
    ...
    def step(self, action):
        ...
        # 포탑 각도 업데이트 및 제한
        self.turret_x = (self.turret_x + turret_dx) % 360.0 # a % b == a - b * (a // b)
        self.turret_y = (self.turret_y + turret_dy) % 360.0
        
        reward = -0.01  # 기본 패널티(시간 지날수록 페널티)
        if self.cooldown_norm 
